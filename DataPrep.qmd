...
# Create a color palette for race.ids
race_ids <- unique(race.entries$race.id)
color_palette <- colorFactor(topo.colors(length(race_ids)), domain = race_ids)
...
# Add circle markers for each combination of year and race.id as separate layers
years <- unique(race.entries$year)
for (year in years) {
  year_data <- race.entries %>% filter(year == !!year)
  for (race_id in unique(year_data$race.id)) {
    ...
    SVracebase <- SVracebase %>%
      addCircleMarkers(
        ...
        color = ~color_palette(race.id), # Color code by race.id
        popup = ~paste("Race ID:", race.id, "<br>Year:", year, "<br>Count:", race.count),
        group = paste("Year:", year, "Race:", race_id)
      )
  }
}

# Add layers control to toggle between the layers for both race.id and year
year_groups <- paste("Year:", years)
race_groups <- paste("Race:", race_ids)

SVrace.map.new <- SVracebase %>%
  addLayersControl(
    overlayGroups = unique(c(year_groups, race_groups)),
    options = layersControlOptions(collapsed = FALSE)
  )
...
# Prepare the simplified_race_entries data frame
simplified_race_entries <- entries %>% 
...
# Create a color palette for simplified_race_ids
simplified_race_ids <- unique(simplified_race_entries$race.id)
simplified_color_palette <- colorFactor(topo.colors(length(simplified_race_ids)), domain = simplified_race_ids)

# Create the base map without any markers
SimplifiedSVracebase <- leaflet() %>% 
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(data = SVpolygon, color = "gray3", weight = 2, fillColor = "gray3", fillOpacity = 0.2)

# Add circle markers for each simplified_race_id as separate layers
for (simplified_race_id in simplified_race_ids) {
  simplified_race_data <- simplified_race_entries %>% filter(race.id == !!simplified_race_id)
  SimplifiedSVracebase <- SimplifiedSVracebase %>%
    addCircleMarkers(
      data = simplified_race_data,
      ~lon, ~lat,
      radius = ~race.count, # Use race.count for the radius
      color = ~simplified_color_palette(race.id), # Color code by race.id
      popup = ~paste("Race ID:", race.id, "<br>Count:", race.count),
      group = paste("Race:", simplified_race_id)
    )
}

# Add layers control to toggle between the layers for simplified_race_id
simplified_race_groups <- paste("Race:", simplified_race_ids)

SimplifiedSVrace.map.new <- SimplifiedSVracebase %>%
  addLayersControl(
    overlayGroups = simplified_race_groups,
    options = layersControlOptions(collapsed = FALSE)
  )

# Print the map
SimplifiedSVrace.map.new
```
